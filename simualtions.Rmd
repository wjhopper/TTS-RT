...
proportion <- outputs_per_time_point/(prod(dim(sim_RT))*mean_acc)
...
curve(dexp(x, rate = coef(est)), add = TRUE)
text(x = params$duration/2, y = max(proportion)*.8,
...
cum_exp <- function(x, y) y - y*exp(-x*coef(est))
...
plot(seq_along(cum_proportion), cum_proportion*mean_acc,
     xlab = "Recall period (s)", ylab = "Proportion Recalled",
     ylim = c(0, 1), main = "Cummulative Latency"
     )
curve(cum_exp(x, y = mean_acc), add = TRUE)
